...
def install_en_title_version(image: bytearray, english_rom: bytes) -> dict:
    ...
    outline = {(x + dx, y + dy) for x, y in pixels
               for dx in (-1, 0, 1) for dy in (-1, 0, 1)} - pixels
    for points, color in ((outline, 15), (pixels, 1)):
        for x, y in points:
    ...
    # Rebase title extension group $0031 from Y=48 to Y=128, so signed
    # offsets can reach the bottom. Compensate every original logo sprite.
    from .asm65816 import assemble
    hook_pc, code_pc = 0xE19E, 0x7FF52
    expected_hook = bytes.fromhex("A5 3B 29 3F 00")
    code = assemble("""
        .a16
        .i16
        php
    ...
    original:
        ...
        and #$003F
        rtl
    """, origin=0xC00000 + code_pc).code
    if image[hook_pc:hook_pc + 5] != expected_hook:
        raise ValueError("EN metasprite dispatch hook mismatch")
    if image[code_pc:code_pc + len(code)] != bytes([255]) * len(code):
        raise ValueError("EN title version code overlaps occupied ROM")
    image[hook_pc:hook_pc + 5] = bytes.fromhex("22 52 FF C7 EA")
    image[code_pc:code_pc + len(code)] = code
    for pc in range(0x7FF22, 0x7FF52, 4):
        if image[pc:pc + 4] != english_rom[pc:pc + 4]:
            raise ValueError("EN title extension metasprite was already modified")
        image[pc + 2] = (image[pc + 2] - 80) & 255
    for pc, tile, _, x in VERSION_SPRITES:
        image[pc:pc + 4] = bytes((tile, 0x10, VERSION_Y - 128, x - 128))
    image[EN_TITLE_LOGO_PC:EN_TITLE_LOGO_PC + EN_TITLE_LOGO_SIZE] = tiles
